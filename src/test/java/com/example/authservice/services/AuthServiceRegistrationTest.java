...
public class AuthServiceRegistrationTest {
    ...
    @BeforeEach
    public void setUp() {
        mockedUser = User.builder().id("TestID").firstName("fName").lastName("LName").email("test@gmail.com").role(Role.USER).build();  // Create a mock user object
    }
    ...
    public void testRegisterWithExistingEmail() {
        ...
        //!Mock
        //? Mock the existsByEmail method within the authRepo. which means whenever the existsByEmail gets executed with the registrationReq.getEmail() as it's param, return true
        when(authRepo.existsByEmail(registrationReq.getEmail())).thenReturn(Boolean.TRUE);

        //! Act and Assert
        //=> This will be our assertion. We will be calling the register method, then we will be checking for the exception
        UserAlreadyExistsException ex = assertThrows(UserAlreadyExistsException.class, () -> service.register(registrationReq));
    ...
    public void testValidRegistration() {
        ...
        //* here we are verifying the interactions, meaning that the mocked methods were actually called or not
        verify(authRepo).existsByEmail(registrationReq.getEmail());
        verify(authRepo).save(any(User.class));
        verify(jwtService).generateToken(any(User.class));
        verify(mailer).sendEmail(
                eq(registrationReq.getEmail()),
                eq("Verify your email address"),
                anyString()
        );
        //* here we verify that aside from the interactions above, no more interactions were made with the authRepo and the jwtService
