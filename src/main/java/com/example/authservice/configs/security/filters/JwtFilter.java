...
public class JwtFilter extends OncePerRequestFilter {
    ...
    private final CustomUserDetailsService userDetailsService; //* already got imps of it, but we need our own imp cuz we need to fetch users from our DB
    @Qualifier("handlerExceptionResolver")
    private final HandlerExceptionResolver resolver;
    private final List<String> openApiEndpoints = Arrays.asList("/api/v1/auth/login", "/api/v1/auth/register", "/error", "/images/.*");

    @Autowired
    public JwtFilter(JwtService jwtService, CustomUserDetailsService userDetailsService, @Qualifier("handlerExceptionResolver") HandlerExceptionResolver resolver) {
        super();
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            if (isOpenEndpoint(request)) {
                logger.trace("Status confirmed: request doesn't require auth... Continuing without token verification");
