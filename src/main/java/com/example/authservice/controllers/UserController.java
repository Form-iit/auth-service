...
public class UserController {
    ...
    public ResponseEntity<Map<String, Object>> EditUserData(@Valid @RequestBody UserDto userData) {
        ...
        return new ResponseEntity<>(MakeResponseObject("user", user, "User modified successfully"), HttpStatus.OK);
    }

    @PatchMapping("/me/edit/password")
    public ResponseEntity<Map<String, Object>> EditUserPassword(@Valid @RequestBody ChangePasswordRequest request) {
        String userId = service.ChangeCurrentUserPassword(request.getOldPassword(), request.getNewPassword());
        return new ResponseEntity<>(MakeResponseObject("Id", userId,  "Password changed successfully"), HttpStatus.OK);
    }

    @DeleteMapping("/me/delete")
    public ResponseEntity<Map<String, Object>> DeleteUserAccount(){
        String userId = service.DeleteCurrentUser();
        return new ResponseEntity<>(MakeResponseObject("Id", userId,"User account was deleted successfully"), HttpStatus.OK);
    }
