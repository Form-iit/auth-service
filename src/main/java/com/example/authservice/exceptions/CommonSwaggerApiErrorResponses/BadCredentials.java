...
@ApiResponse(
        ...
        content = @Content(mediaType = "application/json", schema = @Schema(implementation = GlobalErrorResponse.class),
                examples = {
                        @ExampleObject(name="BadCredentials", description = "This is the error returned when the given credentials (the combination of username/password) are not correct", value = """
                                                    {
