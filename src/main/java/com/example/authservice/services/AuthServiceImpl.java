...
public class AuthServiceImpl implements AuthService {
  ...
  public void register(RegisterRequest request) {
    ...
    try {
      ...
      String publicBaseUrl =
          (serverProtocol + "://" + serviceName + ":" + serverPort).toLowerCase();
  ...
  public void verify() {
    ...
    User user =
        authRepo
            .findById(userId)
            .orElseThrow(() -> new FailedEmailVerification("Failed to verify user"));
  ...
  public String login(AuthRequest request) {
    authManager.authenticate(
        new UsernamePasswordAuthenticationToken(
            request.getEmail(),
            request.getPassword())); // * if this doesn't throw an error, the user is authenticated
